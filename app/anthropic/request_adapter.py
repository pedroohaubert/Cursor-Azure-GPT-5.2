...
class AnthropicRequestAdapter:
    ...
    def adapt(self, req: Request) -> Dict[str, Any]:
        ...
        current_app.logger.info(
            f"[Anthropic Request] Raw tools value: {tools_in_request}"
        )
        current_app.logger.info(
            f"[Anthropic Request] Tools count: {len(tools_in_request) if tools_in_request else 0}"
        )
        current_app.logger.info(
            f"[Anthropic Request] Tools type: {type(tools_in_request)}"
        )
        current_app.logger.info(
        ...
        if tools:
            ...
            current_app.logger.info(
                f"[Anthropic Request] Converted {len(converted_tools)} tools to Anthropic format"
            )
            anthropic_body["tools"] = converted_tools
        else:
            current_app.logger.error(
                "[Anthropic Request] NO TOOLS AFTER AUTO-INJECT - THIS SHOULD NOT HAPPEN!"
            )
